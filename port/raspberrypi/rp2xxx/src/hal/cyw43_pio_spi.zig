...
pub fn init(config: Config) !Cyw43PioSpi {
    ...
    config.pio.set_input_sync_bypass(pin_num(config.io_pin));
    ...
    try config.pio.sm_load_and_start_program(sm, cyw43spi_program, .{
        ...
        .clkdiv = .{ .int = 3, .frac = 0 },
        .pin_mappings = .{
            .out = .{ .base = pin_num(config.io_pin), .count = 1 },
            .set = .{ .base = pin_num(config.io_pin), .count = 1 },
            .side_set = .{ .base = pin_num(config.clk_pin), .count = 1 },
            .in_base = pin_num(config.io_pin),
        },
        .shift = .{
            .out_shiftdir = .left,
            .in_shiftdir = .left,
    ...
    config.pio.sm_set_pindir(sm, pin_num(config.clk_pin), 1, .out);
    config.pio.sm_set_pindir(sm, pin_num(config.io_pin), 1, .out);

    config.pio.sm_set_pin(sm, pin_num(config.clk_pin), 1, 0);
    config.pio.sm_set_pin(sm, pin_num(config.io_pin), 1, 0);
